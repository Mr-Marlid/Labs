...
%1.1
pr1_1:- see('C:/Libs/Labs/Lab8/1.txt'), read_list_str(_, LengthList),
...
%1.2
pr1_2:- see('/1.txt'), read_list_str_space(_, C),
...
%1.3
pr1_3:- see('/completion.txt'), count_A(_,N,C), M is C/N,
	 seen, see('/completion.txt'), read_list_str_A(_,M), seen.
...
%1.4
pr1_4:- see('/completion.txt'), read_list_str1(List), seen,
...
%1.5
pr1_5:- see(/completion.txt'), read_list_str1(List), seen,
	unique_words(List,List,[],R), see('/completion.txt'),
	read_list_str2(_,R,ResList), seen, tell('/out.txt'),
...
%2
pr2_1:- read_str(List,_,_), append([32],List,L), append(L,[32],L1), count_words(L1,0,C),
...
pr2_2:- read_str(A,_), digits(A,D), letters(A,L), append(D,L,List), write_str(List).
...
% 3
pr3_1:-read_str(Str,_,_),pr3(Str,Res),write_list_str(Res).
...
pr3_2(Str,Word,Str2):-get_month(Str,[],Word,Str2).
...
pr4_1:- read_str(A,_), append(A,[32],B), count_5(B,0,C), write("C = "), write(C).
...
pr4_2:- read_str(A,_), cyrillic_char(A,[],B), p4_12(1025,B).
...
% 5
pr5:- see('c:/Users/VirusTM/Documents/Prolog/lab8/completion.txt'), read_list_str(List,LengthList), seen,
...
% 6
pr6:- see('C:/Users/VirusTM/Documents/Prolog/lab8/completion.txt'), read_str_count_words(List,CountWordsList),
...
% 7
pr7:- see('C:/Users/VirusTM/Documents/Prolog/lab8/completion.txt'), read_str_after(List,CountWordsList),
	seen, sort_list(List,CountWordsList,[],SList), write1(SList).
